...
| [BUG-42](https://github.com/kristianwiklund/signalk-weather-routing/issues/106) | `interpolateBoatSpeed` extrapolates without bound when TWS exceeds the highest TWS column in the polar CSV (e.g. 20 kn for sunwind33.pol). `bracketIndex` returns the second-to-last index (`polar.ts:65`), making `tTws = (twsKnots − tws[n-2]) / (tws[n-1] − tws[n-2])` > 1.0 for any wind above that column. For actual wind at 30 kn with a 20 kn polar maximum, `tTws ≈ 3.5`, inflating predicted boat speed roughly 2–3× beyond the highest measured value. ICON-EU files routinely deliver 25–35 kn winds in the Baltic. The router is therefore systematically biased toward any forecast cell where wind exceeds the polar maximum — those cells appear to offer the fastest VMG, so the router routes toward them. Fix: clamp `tTws` to ≤1 so any wind above the polar maximum is treated identically to the polar maximum (the boat goes no faster than the polar's highest measured column). `tTwa` is clamped the same way. Below-minimum TWS is intentionally left unclamped — light-air extrapolation (proportionally slower below the polar's minimum TWS column) is correct behaviour. **Implement together with REQ-45** (max wind speed / wave height constraints): both address how extreme wind conditions are handled and must be tested together. |
