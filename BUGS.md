...
| # | Description |
|---|---|
| [BUG-74](https://github.com/kristianwiklund/signalk-weather-routing/issues/219) | Motoring does not appear to trigger when boat speed falls below the motor threshold, or the conditions graph falsely shows zero boat speed for the affected portion of the route. Observed with motor-below 3 kn / motor-speed 5 kn on the standard Öregrund→Marhällan test (departure 2026-05-24 08:00); rerunning with 6 kn / 6 kn thresholds suggested motoring may trigger correctly, pointing toward a possible rendering artifact in the chart. |
| [BUG-73](https://github.com/kristianwiklund/signalk-weather-routing/issues/218) | The error message "No reachable positions at step 1 — land blocks all paths — All available paths are blocked by land" is shown regardless of the actual cause of route failure. The message should reflect the specific reason for failure rather than always blaming land blockage. |
...
| [BUG-63](https://github.com/kristianwiklund/signalk-weather-routing/issues/221) | The wave overlay does not disappear when the corresponding GRIB file is unticked. |
