...
| # | Description |
|---|---|
| [BUG-135](https://github.com/kristianwiklund/signalk-weather-routing/issues/370) | Multiple ocean-current GRIBs are mishandled because current is treated as single-file (`SingleFileCurrentProvider` loads only the freshest). Observed with 3 norkyst800m current GRIBs: (bug2) clicking any current row in the Grib Manager toggles all of them (one shared global `currentEnabled`); (bug3) no currents visible in the current display; (bug4) no bounding-box rectangles drawn for the non-first current GRIBs (only `currentInfoFiles[0]` gets a bbox). Surfaced during REQ-139 upload testing; logged only — not investigated/fixed without confirmation. |
