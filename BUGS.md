...
| [BUG-84](https://github.com/kristianwiklund/signalk-weather-routing/issues/260) | The scrubber time range does not update when individual GRIB files are activated or deactivated via their checkboxes. The range stays fixed at the value computed when the files were first loaded. |
...
| [BUG-82](https://github.com/kristianwiklund/signalk-weather-routing/issues/257) | Current overlay shows physically impossible current values (e.g. 27487.3 kn at 45°T) over land when an RTOFS GRIB is loaded. Expected: out-of-domain or fill-value cells should not be rendered. |
