...
| # | Description |
|---|---|
| [BUG-85](https://github.com/kristianwiklund/signalk-weather-routing/issues/261) | When all wind GRIBs are unchecked but a current GRIB remains checked, dragging the scrubber still renders wind data on the wind overlay. The wind layer should show nothing when all wind files are deselected. |
| [BUG-84](https://github.com/kristianwiklund/signalk-weather-routing/issues/260) | The scrubber time range is broken in general and needs an overhaul. Known symptoms: (1) range does not update when individual GRIB files are activated or deactivated; (2) current GRIB times are not included in the range; (3) the REQ-91 test case for scrubber + current times could not be completed due to general scrubber instability. |
...
| [BUG-82](https://github.com/kristianwiklund/signalk-weather-routing/issues/257) | Current overlay shows physically impossible current values (e.g. 27487.3 kn at 45°T) over land when an RTOFS GRIB is loaded. Expected: out-of-domain or fill-value cells should not be rendered. **Troubleshooting:** `readCurrentGrib` reads raw band data without filtering fill values. BSH and RTOFS current GRIBs use `noDataValue = 9999` for land/missing cells (confirmed via gdal-async `band.noDataValue` and raw pixel inspection). Bilinear interpolation then operates on these fill values, producing huge intermediate results near land boundaries. Same pattern as wave data (fixed with `>= 100` threshold at usage point). **Fix:** zero out cells where `rawU[i] >= 9000` or `rawV[i] >= 9000` at load time in `readCurrentGrib`, before storing into `Float32Array`. Also skip rendering in `/current-grid` endpoint when `u*u + v*v` exceeds a physical maximum (belt-and-suspenders). |
