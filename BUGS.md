...
| [BUG-121](https://github.com/kristianwiklund/signalk-weather-routing/issues/324) | The `reduce` pattern for finding the closest frontier point to the destination appears 3 times in `isochrone.ts` (lines 305-307, 346-348, 355-358). Discovered in code review 2026-06-14 (m3). |
...
| [BUG-119](https://github.com/kristianwiklund/signalk-weather-routing/issues/322) | Mixed use of `'node:fs'` / `'fs'` for the same module. `polar.ts` and `setup.ts` use `'fs'`; rest of codebase uses `'node:fs'`. `parsePolar` uses `readFileSync` in an otherwise async context. Discovered in code review 2026-06-14 (m1). |
| [BUG-118](https://github.com/kristianwiklund/signalk-weather-routing/issues/321) | `dilate.ts` is not imported by any runtime code. The runtime dilation worker was removed (BUG-29/REQ-51); dilation is now done by the Python build script. The only consumer is `dilate.test.ts`. The comment at line 95 claiming "called from setup.ts" is stale. Reclassified from prior M4. |
| [BUG-117](https://github.com/kristianwiklund/signalk-weather-routing/issues/320) | Multiple `innerHTML` assignments in `public/index.html` insert data from external sources (chart names, region names, GRIB metadata) without HTML escaping. The frontend already uses `textContent` in 29 other locations. Discovered in code review 2026-06-15 (Nm5). |
| [BUG-116](https://github.com/kristianwiklund/signalk-weather-routing/issues/319) | `loadRegions()` auto-cleans stale UUIDs and persists config as a side effect of a read operation. Called from `start()`, `/calculate`, `/reload-grib`, and `/avoid-regions` — each could rewrite config. Discovered in code review 2026-06-15 (Nm4). |
...
| [BUG-105](https://github.com/kristianwiklund/signalk-weather-routing/issues/308) | `console.warn` used for GRIB file load failure (`index.ts:431`) instead of `app.debug`. In SignalK server context `console.warn` may not be visible to the end user. Discovered in code review 2026-06-14 (M3). |
| [BUG-104](https://github.com/kristianwiklund/signalk-weather-routing/issues/307) | `getWaveAt` (`grib.ts:296-310`) doesn't validate lat/lon bounds before bilinear interpolation. `bilinear()` silently clamps out-of-domain coordinates to edge values. Wave lookups for `maxWaveM` constraint have no bounds check. Discovered in code review 2026-06-15 (NM5). |
| [BUG-103](https://github.com/kristianwiklund/signalk-weather-routing/issues/306) | `loadRegions` auto-clean fires `app.savePluginConfig()` without `await` (`index.ts:142`). Unhandled rejection risk; races with `/avoid-regions` PUT handler which also saves config. Discovered in code review 2026-06-15 (NM4). |
| [BUG-102](https://github.com/kristianwiklund/signalk-weather-routing/issues/305) | Invalid `departureTime` string (e.g. "garbage") produces `NaN` from `new Date().getTime()`, causing empty GRIB filter and misleading "No GRIB files cover the requested departure time" error. The real issue is an invalid date. Discovered in code review 2026-06-15 (NM3). |
| [BUG-101](https://github.com/kristianwiklund/signalk-weather-routing/issues/304) | `getWave` in `windprovider.ts:64-71` has the same three-level silent fallback as BUG-93 but for wave data. Wrong wave height could be used for the `maxWaveM` routing constraint. Nautical Safety Rule concern. Discovered in code review 2026-06-15 (NM1). |
| [BUG-100](https://github.com/kristianwiklund/signalk-weather-routing/issues/303) | Waypoint GRIB coverage check (`index.ts:408-420`) is nested inside the `if (useLandAvoidance && activeIndex)` guard. When land avoidance is disabled, waypoints outside GRIB coverage pass silently, causing confusing mid-route failure. GRIB coverage is independent of land avoidance. Discovered in code review 2026-06-15 (NC2). |
| [BUG-99](https://github.com/kristianwiklund/signalk-weather-routing/issues/302) | `/avoid-regions` PUT validation (`index.ts:841`): `ids.filter(id => valid.has(id) || !valid.size)` accepts ALL UUIDs when no regions are loaded (`valid.size === 0` → `!valid.size` is true). Silently persists unvalidated input to plugin config. Discovered in code review 2026-06-15 (NC1). |
| [BUG-98](https://github.com/kristianwiklund/signalk-weather-routing/issues/300) | The hires variants of the GSHHG data (based on the "f" variant) looks identical to the normal (based on the "h" variant). **Troubleshooting:**
- Downloaded `edge-index-hires.bin.gz` and `dilated-edge-index-hires.bin.gz` from the separate repo (`https://github.com/kristianwiklund/weather-routing-hires-land-data`).
- Compared file sizes: both hires files are **exactly** the same size as the normal h-tier files in `data/` (edge-index: 22,165,976 bytes, dilated-edge-index: 78,562,933 bytes).
- MD5 confirmed they are **byte-for-byte identical** (`0fb8e736...` for edge-index, `9b2cad46...` for dilated).
- The `scripts/prepare-land-data.py` has a hardcoded `GSHHG_RESOLUTION = 'h'` (line 31) and always outputs `edge-index.bin.gz` / `dilated-edge-index.bin.gz` — there is no automated mechanism to produce the `-hires` variants.
- **Root cause:** When the hires files were built, `GSHHG_RESOLUTION` was never changed from `'h'` to `'f'`. The normal h-tier output files were simply renamed to `-hires` and uploaded to the separate repo. The plugin detects and loads the hires files correctly, but the data is identical, so the hires overlay looks the same as normal.
- **Fix (Phase 1):** `prepare-land-data.py` now accepts `-r f` CLI argument; `_output_name()` maps f-tier to `-hires` suffix matching `setup.ts` expectations. Genuine f-tier binaries built and uploaded as release assets to `weather-routing-hires-land-data` repo (v1.0.0). Old broken files removed from that repo. File sizes confirm f-tier is genuinely different: edge-index 87 MB (vs 22 MB h-tier), dilated 160 MB (vs 79 MB h-tier). MD5s differ. |
