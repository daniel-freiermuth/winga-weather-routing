...
| # | Description |
|---|---|
| [BUG-75](https://github.com/kristianwiklund/signalk-weather-routing/issues/225) | Press the "run test" button with all test GRIBs (May 24 and June 6) active — the system routes part of the route outside the boundary of the May 24 GRIB and uses the June 6 GRIB instead. **Troubleshooting:** `MultiFileWindProvider.coversPoint()` (windprovider.ts:75) checks spatial coverage only — no time check. The isochrone uses it at line 216 to gate candidates; a point outside the May 24 GRIB's bbox but inside the June 6 GRIB's bbox incorrectly passes. `selectFile()` then falls back to its spatial-only path (line 46) and picks the June 6 GRIB for a May 24 time step. Fix: add `coversPointAtTime(lat, lon, timeIdx)` which checks both space and time; use it in the isochrone check instead of `coversPoint`. |
