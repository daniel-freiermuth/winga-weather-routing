...
| # | Description |
|---|---|
| [BUG-37](https://github.com/kristianwiklund/signalk-weather-routing/issues/99) | Start 57.5512°N 11.6235°E → destination 58.5052°N 17.3474°E, departure 2026-06-06 16:00, current test GRIBs: the isochrone frontier reaches Öresund and then the algorithm cannot exit — the system enters a broken state. User hypothesis: either the polar produces zero or near-zero speeds for the headings available through the strait, or the 90° coarse-pass cone pruning (REQ-35) eliminates all candidates east of the start-to-destination bearing once the frontier is inside Öresund, leaving the fine pass with nothing to expand. Exact failure mode (exception, UI hang, garbled status) not yet confirmed. |
