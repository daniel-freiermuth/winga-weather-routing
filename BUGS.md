...
| [BUG-46](https://github.com/kristianwiklund/signalk-weather-routing/issues/110) | The isochrone seed point is never checked against `wind.coversPoint(start.lat, start.lon)`. If the departure is outside the GRIB domain, `wind.getWind()` silently falls back to `sortedFiles[0]` and `bilinear()` clamps the out-of-domain query to the nearest grid edge. The router proceeds with extrapolated wind data at the departure point with no error or warning. The `wind.coversPoint()` guard at `isochrone.ts:132` protects candidate positions but not the initial seed. |
...
| [BUG-42](https://github.com/kristianwiklund/signalk-weather-routing/issues/106) | `interpolateBoatSpeed` extrapolates without bound when TWS exceeds the polar maximum. `bracketIndex` returns the second-to-last index (`polar.ts:65`), making `tTws = (twsKnots − tws[n-2]) / (tws[n-1] − tws[n-2])` > 1.0. For a polar with max at 20 kn and actual wind at 30 kn, `tTws ≈ 3.5`, inflating predicted boat speed far beyond measured values. ICON-EU files routinely deliver 25–35 kn winds. The router is therefore systematically biased toward high-wind zones, which appear to offer the best VMG — the opposite of conservative routing. Safety-relevant: the boat may be routed into conditions more severe than the skipper chose. Fix: clamp `tTws` and `tTwa` to [0, 1] so the bilinear interpolation never leaves the measured data quadrant. **Implement together with REQ-45** (max wind speed / wave height constraints): both address how extreme wind conditions are handled and must be tested together. |
| [BUG-39](https://github.com/kristianwiklund/signalk-weather-routing/issues/103) | `MultiFileWindProvider` selects the GRIB file with the highest mtime that covers a point spatially, without checking whether that file's time range covers the requested route time. When a newer file with a non-overlapping forecast period is present alongside an older file for the same area, all wind lookups in that area use the nearest time step in the newer file — producing wind data from the wrong forecast entirely. Observed: with `Baltic_Centre_20260524` (May 24–29) and `Baltic_Centre_20260606` (June 6–11) both in the GRIB directory, routing a May 24 departure uses June 6 00:00 wind data for the entire Baltic Centre region. OpenCPN loading the same May 24 file shows different (correct) wind arrows. |
| [BUG-38](https://github.com/kristianwiklund/signalk-weather-routing/issues/102) | Unticking the land overlay checkbox does not remove the overlay from the map — this occurs regardless of whether the safety margin is enabled. When safety margin is on, the light gray safety margin layer remains; when safety margin is off, the dark gray land overlay remains. |
