...
| # | Description |
|---|---|
| [BUG-37](https://github.com/kristianwiklund/signalk-weather-routing/issues/99) | Likely same root cause as BUG-34 — fix together. Start 57.5512°N 11.6235°E → destination 58.5052°N 17.3474°E, departure 2026-06-06 16:00: calculation terminates "partial route shown (8958 nm from destination)." **Root cause confirmed by analysis — three independent defects:** (1) The 90° coarse-pass cone (REQ-35) prunes all southward candidates (going south deviates 125° from the NE start→end bearing), so the coarse pass cannot traverse Öresund and returns T_bound=null. (2) With T_bound=null the fine pass runs unconstrained, and `pruneToFrontier`'s "farthest from start" criterion keeps frontier points that escaped south past the GRIB domain boundary over correctly-navigating points that threaded passages. This is not a land-detection bug: `pruneToFrontier` only sees candidates that already cleared the land check — both the escaped point and the channel-threading point are valid water routes; the wrong one wins purely because of the distance-from-start comparator. The correct criterion is the A* `g+h` value: minimum estimated total arrival time `point.time + haversineNM(point,end)/maxBoatSpeed`. (3) No domain boundary check exists: points outside the GRIB coverage area get clamped boundary wind (non-zero) from `bilinear`, generate real-looking boat speed, and the frontier propagates indefinitely outside the Baltic — hence 8958 nm. **Fixes applied:** (A) Remove the 90° cone from `runCoarsePass` — restores T_bound on circuitous routes and re-enables T_bound pruning in the fine pass. (B) Discard candidates outside GRIB domain in both loops. (D) Coarse pass: skip empty steps rather than aborting. Fix (C) — replace distSq with A* g+h — was implemented then reverted: in the isochrone algorithm all step-N candidates share the same `g` value, so g+h reduces to min-haversine-to-destination per sector. For routes requiring a southward detour, min-h keeps near-start points (smaller haversine to the NE destination) over south-advancing points, pinning the frontier near the start indefinitely (confirmed by reading route "test 1": all 79 waypoints within 20 nm of Gothenburg over 107 hours). Reverted to distSq; domain check (B) prevents frontier escape; T_bound from fixed coarse pass (A) eliminates escaped distSq winners. **Test result (2026-06-06):** Gothenburg→Stockholm route now completes. Phase 1 isochrones correctly traverse Öresund; frontier clipped at GRIB boundary in the North Sea; T_bound pruning holds phase 2 frontier near the Falsterbo–Rügen line. Completed route navigates around Bornholm and turns east along the Estonian coast on 2026-06-08 18:00 — assessed as reasonable given wind conditions. |
| [BUG-38](https://github.com/kristianwiklund/signalk-weather-routing/issues/102) | Unticking the land overlay checkbox does not remove the light gray safety margin overlay from the map. |
