...
| # | Description |
|---|---|
| [BUG-72](https://github.com/kristianwiklund/signalk-weather-routing/issues/216) | When `windSpeedMs` is enabled, the conditions graph dual-axis layout is broken: the additional outer left axis for TWS in m/s is missing, and the overall graph layout is visually incorrect. The wind unit label on the axis shows the correct unit (m/s), but the second axis tick marks and its labels are not rendered. |
| [BUG-71](https://github.com/kristianwiklund/signalk-weather-routing/issues/212) | Unit preferences not applied in the frontend. When the SignalK active unit preference is set to metric or imperial/US, all velocity units in input fields and labels remain as knots, and the safety margin distance is still displayed as NM. Expected: all frontend unit labels update to match the selected unit preset. **Troubleshooting:** Two bugs found and fixed in `loadConfig()`: (1) `unitPrefs` was set to the full API response object instead of `.categories` — `unitPrefs?.speed` was always `undefined`, forcing fallback to kn; (2) `displayFormat` parsing used `parseInt("0.0")` → 0 instead of counting digits after the decimal point. Both fixed. **Further finding (confirmed 2026-06-13):** `/signalk/v1/unitpreferences/active` returns the server-wide default, not the per-user preference. Units only update in the webapp when the preference is set as "default for new users" in SignalK — changing a per-user preference has no effect on what the endpoint returns. This is SignalK API behaviour, not a fetch/parse bug. **Remaining gap:** The safety margin label (`0.5 NM`) is hardcoded and not covered by unit conversion — it always shows NM regardless of the active preset. |
| [BUG-70](https://github.com/kristianwiklund/signalk-weather-routing/issues/211) | Route calculation changes to a different path after REQ-99 implementation. The standard test (Öregrund → Gotska Sandön) now routes north around Åland via the Finland side instead of through Ålandförträngningen. Observed with the nautical-metric preset. The routing algorithm was not intended to change (REQ-99 is a frontend-only change). |
