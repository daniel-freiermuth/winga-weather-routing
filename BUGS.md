...
| [BUG-79](https://github.com/kristianwiklund/signalk-weather-routing/issues/241) | Plugin fails to start on ARM64 (Raspberry Pi 3, linux-arm64): `Cannot find module '.../gdal-async/lib/binding/node-v137-linux-arm64/gdal.node'`. The bundled gdal-async binary is built for linux-x64 (the development server); no ARM64 binary is included. Downstream symptoms on the clean RPi install: (1) plugin does not appear in SignalK plugin config settings; (2) "run test" fails with "could not open event stream: stream connection failed" — the plugin API routes are never registered. |
