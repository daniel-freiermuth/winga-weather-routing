...
| # | Description |
|---|---|
| [BUG-37](https://github.com/kristianwiklund/signalk-weather-routing/issues/99) | Likely duplicate of BUG-34 — investigate together. Start 57.5512°N 11.6235°E → destination 58.5052°N 17.3474°E, departure 2026-06-06 16:00, current test GRIBs: the calculation terminates with "Route extends past forecast coverage — partial route shown (8958 nm from destination)." The reported closest-approach distance of 8958 nm is impossibly large for a Baltic route (Stockholm is ~500 nm from Gothenburg by sea), indicating the isochrone frontier has drifted to positions far outside the Baltic before the forecast data runs out. User hypothesis: either the polar produces zero or near-zero speeds for headings through Öresund causing erratic frontier expansion, or the 90° coarse-pass cone pruning (REQ-35) eliminates all geographically sensible candidates once the frontier is inside the strait, leaving the fine pass to expand unconstrained in wrong directions. |
