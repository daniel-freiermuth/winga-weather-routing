...
| [REQ-106](https://github.com/kristianwiklund/signalk-weather-routing/issues/217) | **Original:** «When a wave overlay is active it is possible to click the layer to get wave information. This was introduced for troubleshooting of #197. However, this is generally useful, add it as an always-available feature covering all layers on the map.»<br>**Interpretation:** The click-to-show-info behaviour currently implemented on the wave canvas overlay is made permanent and general: clicking anywhere on the map shows the wind speed/direction and wave height at that point from the active GRIB data, regardless of which overlays are ticked. | open |
