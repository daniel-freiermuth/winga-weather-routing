...
| # | Requirement | Status |
|---|---|---|
| [REQ-114](https://github.com/kristianwiklund/signalk-weather-routing/issues/254) | **Original:** «make the "Land overlay" tickbox label say "Land overlay (hires)" if hires is installed.»<br>**Interpretation:** When the hires land index files are detected at startup, the "Land overlay" checkbox label in the routing UI is rendered as "Land overlay (hires)" to indicate that the f-tier coastlines are active. When standard h-tier data is in use, the label remains "Land overlay". | open |
| [REQ-113](https://github.com/kristianwiklund/signalk-weather-routing/issues/253) | **Original:** «No, we will do an optional package from it.»<br>**Interpretation:** Support for hires landmass as optional separate package. Distribute the GSHHG `f`-tier (full-resolution, ~100 m) pre-built land index files (`edge-index-hires.bin.gz` and `dilated-edge-index-hires.bin.gz`) from [weather-routing-hires-land-data](https://github.com/kristianwiklund/weather-routing-hires-land-data) as a separate optional npm package, following the same pattern as `@kristianwiklund/wr-gdal-linux-*`. The main plugin continues to bundle the `h`-tier indices for baseline land avoidance. If the optional package is installed, the plugin uses the `f`-tier indices instead. This avoids bloating the main package with the `f`-tier data. | open |
| [REQ-109](https://github.com/kristianwiklund/signalk-weather-routing/issues/228) | **Original:** «The current land avoidance mask was added during the 1st trials with a much slower algorithm. Investigate if we can use the next step resolution with reasonable performance.»<br>**Interpretation:** Investigate upgrading from the GSHHG `h` (high) resolution tier to the `f` (full) resolution tier for the land avoidance mask. The edge-tile index (added since the original land mask implementation) may make this feasible. A performance study is required before any change is committed. | open |
