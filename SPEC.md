...
| [REQ-112](https://github.com/kristianwiklund/signalk-weather-routing/issues/243) | **Original:** «seems pointless to rebuild these binaries every time we do a new release? We should be able to put them in a new separate repo?»<br>**Interpretation:** Create a separate repository with a standalone workflow that builds gdal-async native binaries once per GDAL/ABI change and publishes them as a GitHub Release. The main plugin's publish workflow downloads the prebuilt assets by version tag instead of rebuilding from source on every tag push. | open |
