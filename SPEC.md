...
| [REQ-32](https://github.com/kristianwiklund/signalk-weather-routing/issues/32) | Weather data can be loaded from multiple GRIB files, merged into a single forecast covering a larger time range or geographic area | open |
| [REQ-44](https://github.com/kristianwiklund/signalk-weather-routing/issues/44) | The webapp has a "Clear" button that removes the currently displayed route (route line, waypoint labels, wind barbs, isochrone lines) from the map. | open |
