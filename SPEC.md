...
| [REQ-99](https://github.com/kristianwiklund/signalk-weather-routing/issues/182) | **Original:** «(a) The plugin displays and handles data entry using the unit preferences retrieved from signalk. (b) If the user ticks a tick-box in the plugin configuration, WIND SPEED specifically, and ONLY wind speed, is handled as m/s. (c) No changes are done in the backend algorithms, this is a front-end/UX change»<br><br>**Interpretation:** (a) The webapp fetches the active unit preset from `GET /signalk/v1/unitpreferences/active` on load and uses it to convert and label all displayed values (TWS, boat speed, wave height, distances) and all numeric data entry fields that carry units (comfort constraints, motor speed, motor threshold, and any other unit-bearing inputs). All hardcoded unit labels (kn, m, nmi) are replaced with dynamic values from the preset. (b) The SignalK plugin config gains a boolean setting (e.g. `windSpeedMs`, default false). When enabled, wind speed (TWS) is displayed and entered in m/s everywhere in the webapp, overriding whatever the active unit preset says for the speed category. All other values continue to follow the preset. (c) The routing algorithm, GRIB processing, and polar interpolation are not touched. All changes are confined to the frontend. | open |
