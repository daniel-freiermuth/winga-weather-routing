...
| [REQ-32](https://github.com/kristianwiklund/signalk-weather-routing/issues/32) | Weather data can be loaded from multiple GRIB files to cover a larger time range and/or geographic area. Files with identical grids are merged automatically into a single time-extended dataset. Files with different grids are kept as separate sources; wind/wave lookups route to whichever source covers the requested position. Each file's bounding box is shown on the map as a separate dashed rectangle, rotating through a C64-inspired color palette. | open |
