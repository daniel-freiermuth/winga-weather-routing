...
| # | Requirement | Status |
|---|---|---|
| [REQ-118](https://github.com/kristianwiklund/signalk-weather-routing/issues/267) | **Original:** «"now" marker on the scrubber shows current wall-clock time relative to GRIB data, and a jump-to-now button snaps the scrubber to the closest available timestep to the current time»<br>**Interpretation:** Display a visual tick or marker on the scrubber track at the position corresponding to the current wall-clock time. Add a jump-to-now button that sets the scrubber to the nearest available timestep to `Date.now()`. Gives the sailor an immediate reference point ("I am here") without mental arithmetic from the GRIB start time. | open |
| [REQ-117](https://github.com/kristianwiklund/signalk-weather-routing/issues/266) | **Original:** «scrubber track shows GRIB coverage regions in different shading»<br>**Interpretation:** The scrubber track is colour-coded by data availability at each time position: one shade for wind-only coverage, another for current-only, another for both, and another for gaps. Gives the sailor at-a-glance visibility of which forecast types are available across the time range without having to compare file rows in the sidebar. | open |
| [REQ-116](https://github.com/kristianwiklund/signalk-weather-routing/issues/262) | **Original:** «"move to grib time", click something in the grib list, have scrubber time go there»<br>**Interpretation:** Clicking a GRIB file entry in the sidebar (wind or current) moves the scrubber to the start time of that file's forecast range. | open |
