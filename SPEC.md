...
| [REQ-110](https://github.com/kristianwiklund/signalk-weather-routing/issues/230) | **Original:** «Add a warning box to the first run of the plugin clarifying that this is an experimental solution, that routes cannot be guaranteed to not cross land or shallow water, and that weather conditions change (etc) - using the plugin does not replace good seamanship and is on the users own riskl - research if there is a similar warning/info statement somewhere. Also add the same to the README»<br>**Interpretation:** Display a one-time dismissible warning dialog on first app load (dismissed state stored in localStorage): the plugin is experimental, routes may cross land or shallow water, weather conditions change, the plugin does not replace good seamanship, use is at the user's own risk. Research whether other marine navigation tools use a standardised disclaimer. Add equivalent text to README. | open |
