...
| [REQ-35](https://github.com/kristianwiklund/signalk-weather-routing/issues/35) | During the coarse pre-pass, each candidate is checked before being added to the frontier: discard any candidate whose bearing from the start deviates by more than 90° from the direct start→destination bearing. This cone-prunes the pre-pass at generation time so that candidates heading away from the destination are rejected immediately, producing visually meaningful cone-shaped isochrones rather than full rings. The 90° half-angle allows full tacking coverage while eliminating candidates in the opposite hemisphere from the destination. | done |
...
| D13 | `pruneToFrontier` uses the A* `g+h` criterion — minimum estimated total arrival time `point.time + haversineNM(point,end)/maxBoatSpeed` — in **both** the coarse pass and the fine pass. The alternative (apply `g+h` only in the fine pass, leave the coarse pass using distSq-from-start) was considered: it would avoid adding `time` to `CoarsePoint` and is safe because the coarse pass only establishes T_bound rather than the route. It was rejected because using an incorrect dominance criterion in the coarse pass can prune away the only frontier path that would reach the destination, producing a pessimistic or null T_bound that degrades fine-pass performance. Consistency between passes is the safer default; if performance regression is observed in the coarse pass the fallback is to revert the coarse pass to distSq while keeping `g+h` in the fine pass. |
