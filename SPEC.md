...
| [~~REQ-121~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/279) | ~~**Original:** «since it takes some time to load the grib data to the gui, on gui startup, we need a "loading" indicator.»<br>**Interpretation:** A visual loading indicator (spinner, progress bar, or similar) is shown in the GUI while GRIB data is being loaded and indexed at startup. The indicator is removed once all GRIB data has been loaded and the UI is ready for interaction.~~ — **done** (CSS spinner shown in `#grib-info` during initial load; status box shows "Loading wind data [spinner]" during `initWindScrubber`, replaced with "Ready" on completion; confirmed 2026-06-14) | done |
