...
import { evalFormula, fmt, parseUnit, toDisplay, toSI, type UnitPrefs } from '../unitconversion';
...
void test('fmt: speed kn→mph with imperial-us preset', () => {
...
void test('fmt: depth m→ft with imperial-us preset', () => {
...
void test('fmt: speed kn→km/h with metric preset', () => {
...
void test('fmt: distance nmi→km with metric preset', () => {
...
void test('fmt: speed falls back to kn when prefs null', () => {
  const r = fmt(10, 'speed', null);
  assert.equal(r.sym, 'kn');
  assert.equal(r.num, '10.0');
});

void test('fmt: depth falls back to m when prefs null', () => {
  const r = fmt(3, 'depth', null);
  assert.equal(r.sym, 'm');
  assert.equal(r.num, '3.0');
});

void test('fmt: distance falls back to nmi when prefs null', () => {
...
void test('fmt: forceMs=true converts kn to m/s', () => {
...
void test('fmt: forceMs=true ignores preset', () => {
...
void test('parseUnit: speed nautical-metric round-trip', () => {
